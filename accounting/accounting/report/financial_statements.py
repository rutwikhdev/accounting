...
def get_data(root_type, balance_must_be, from_date, to_date, total = True):
    ...
    for root in frappe.db.sql("""select lft, rgt from tabAccounts
            where root_type=%s and ifnull(parent_accounts, '') = ''""", root_type, as_dict=1):

        set_gl_entries_by_account(from_date, to_date, root.lft, root.rgt, gl_entries_by_account)

    calculate_values(accounts_by_name, gl_entries_by_account, to_date)
...
def get_accounts(root_type):
    return frappe.db.sql("""
        select name, account_number, parent_accounts, lft, rgt, root_type, report_type, account_name, account_type, is_group, lft, rgt
        from `tabAccounts`
...
def set_gl_entries_by_account(from_date, to_date, root_lft, root_rgt, gl_entries_by_account):
    ...
    if accounts:
        additional_conditions += "account in ({})".format(", ".join(frappe.db.escape(d) for d in accounts))

    query = """select posting_date, account, debit, credit from `tabGL Entry`
    ...
    # print('GL Entries in SET gl entries: ', gl_entries)
    # print('GL Entries by account: ', gl_entries_by_account)
...
def calculate_values(accounts_by_name, gl_entries_by_account, to_date):
    for entries in itervalues(gl_entries_by_account):
        for entry in entries:
            ...
            if not d:
                frappe.msgprint(_("Could not retrieve information for {0}.").format(entry.account), title="Error", raise_exception=1)
...
def add_total_row(out, root_type, balance_must_be):
    ...
    for row in out:
        if not row.get("parent_accounts"):
...
def prepare_data(accounts, balance_must_be, from_date, to_date):
    ...
    for d in accounts:
        ...
        row = frappe._dict({
            ...
            "parent_accounts": frappe._(d.parent_account) if d.parent_account else '',
            "indent": flt(d.indent),
